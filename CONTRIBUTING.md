...
4. If you add a skill or template, add a row for it to `skills/ai-brain/references/skill-catalog.md` (and a recipe in `deliverable-recipes.md` if it produces a new deliverable), so the AI Brain can route people to it.
5. Run `python scripts/validate_skills.py` and fix anything it reports. It fails if the AI Brain catalog is missing a skill or template.
6. Confirm your change adds no private or sensitive content (see above).
7. Open a pull request describing what the change improves and why.
